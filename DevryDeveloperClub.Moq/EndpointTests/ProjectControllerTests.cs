...
namespace DevryDeveloperClub.Moq.EndpointTests
{
    ...
    /// </summary>
    public class ProjectControllerTests : BaseControllerTest<ProjectController, Project, CreateProjectDto>
    {
        ...
        public override async Task CreateTag_WithInvalidData()
        {
            ...
            var allInvalid = await (dynamic)Sut.Post(new CreateProjectDto());

            Assert.Equal(400, allInvalid.StatusCode);
            Assert.Equal(3, allInvalid.Value.Length);
            
            // Should get 2 error messages
            var twoInvalid = await (dynamic)Sut.Post(new CreateProjectDto() { Name = "asdf" });
            Assert.Equal(400, twoInvalid.StatusCode);
            Assert.Equal(2, twoInvalid.Value.Length);
            
            // Should get 1 error message
            var oneInvalid = await (dynamic)Sut.Post(new CreateProjectDto() { Name = "asdf", GithubLink = "asdfasdfasd" });
